...
> 仓库：`https://github.com/YardRat0117/ctop-plus-plus`
...
| 成员 | 姓名 | 学号 | 负责模块 | 交付物 |
|------|------|------|---------|--------|
| **同学 A** | 何佳毅 | 3240104782 | `SysStatsModel` + `SysViewModel` | `/proc` 文件读取、CPU/内存/磁盘/网卡数据采集、聚合逻辑 |
| **同学 B** | 林书帆 | 3240103707 | 系统资源面板 UI 初步实现 | 交付 `.cpp` 实现文件（参见 `docs/archive/contrib-b/`） |
| **同学 C** | 于睿 | 3240105781 | eBPF 探针 + `NetworkModel` + `NetViewModel` + 工程配置 | BPF C 程序、libbpf 桥接、网络数据聚合、CMake/vcpkg |

**开发流程：** C 先行定义全部接口头文件（数据契约），A 与 C 并行开发各自模块，B 提交 PR 后由 C 集成修正。完成后通过 squash merge 合入 `main` 分支。
...
- 前端 UI（系统资源面板）：100% 完成，由同学 C 集成并修复编译问题
...
同学 B 负责系统资源面板（System Stats Tab）的 UI 开发，交付物已归档至 `docs/archive/contrib-b/`，包括一个微信发送的 docx 心得文件和一个 `sys stats前端UI.cpp`。后者以无后缀的 `sys states` 文件名通过 PR #7（commit `08cb1d8`）提交到项目根目录，未替换 `main.cpp` 中的占位函数。

该代码使用了 ImPlot 1.0 已废弃的 `SetNextLineStyle()` API 导致编译失败，且缺少必要的头文件。同学 B 未参与后续团队协作，未使用 AI 工具。以上问题由同学 C 在 PR 基础上修正：合入 `main.cpp`、替换废弃 API 为 `ImPlotSpec` 属性对写法、补充头文件依赖并验证编译通过。

网络流量面板的全部 UI（包数据表格、吞吐量折线图、Top IP 列表、协议饼图、ImGuiListClipper 视口优化、ImPlot 1.0 API 适配）及 MVVM 数据契约定义、网络管线（eBPF 探针 + NetworkModel + NetViewModel）的设计实现，均由同学 C 独立完成。

同学 B 的原始 docx 中关于 ImPlot 适配、渲染优化及 AI 工具使用的描述与实际分工严重不符，不予采信。
